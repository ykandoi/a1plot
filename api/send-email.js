...
export default async function handler(req, res) {
  ...
  const { adminEmail, userEmail, userName, plotId, plotTitle, plotLocation, plotPrice, plotSize } = req.body;
  ...
  if (!smtpUser || !smtpPass) {
    ...
    return res.status(200).json({
      success: true,
      info: "Mock email printed to server console successfully. Setup SMTP_USER and SMTP_PASS to send live emails.",
      mock: true
    });
  }
  ...
  try {
    ...
    console.log("✅ Live email sent successfully:", info.messageId);
    ...
    console.error("❌ Error sending live email via Nodemailer:", error);
