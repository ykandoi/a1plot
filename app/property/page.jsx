...
export async function generateMetadata({ searchParams }) {
  ...
  return {
    title, description,
    alternates: { canonical: `https://a1plot.com/property?id=${id}` },
    openGraph: { title, description, url: `https://a1plot.com/property?id=${id}`, type: 'website', ...(image ? { images: [{ url: image }] } : {}) },
...
export default async function Page({ searchParams }) {
  ...
  // In-app reload without ?id= — resolve from localStorage client-side.
  if (!id) {
